...
public class Pen : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!Holding && Is_out())
        {
            ...
            this.transform.position = Position_Start + new Vector3(0,0.3f,0);
    ...
    {
        if(other.gameObject.tag == "Holder" && !Holding)
        {
            //Debug.Log(other.transform.position - this.transform.position);
            this.transform.position = other.transform.position - new Vector3(0, -0.11f, 0.04f);
            rb.velocity = Vector3.zero;
            this.transform.localRotation = Quaternion.Euler(180, 180, 45);
